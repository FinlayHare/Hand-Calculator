...
while True:
    ...
    if hands:
        ...
        if len(lmList1) != 0:
            ...
            elif lmList1[17][1] > lmList1[2][1]:  # One finger
                current_detection = "6"
            elif fingers == [1, 1, 0, 0, 0]:  # Two fingers
                current_detection = "7"
            elif fingers == [1, 1, 1, 0, 0]:  # Three fingers
                current_detection = "8"
            elif fingers == [1, 0, 1, 1, 1]:  # Four fingers
                current_detection = "9"
            elif fingers == [0, 0, 0, 0, 0]:  # Five fingers
                current_detection = "0"
            elif fingers == [0, 1, 0, 0, 1]:  # Index and pinky (e.g., '+')
                current_detection = "+"
            elif fingers == [0, 0, 0, 0, 1]:  # Index and pinky (e.g., '+')
                current_detection = "-"
            elif fingers == [0, 1, 0, 1, 1]:  # Index and pinky (e.g., '+')
                current_detection = "*"
            elif fingers == [0, 1, 1, 0, 1]:  # Index and pinky (e.g., '+')
                current_detection = "/"
                
                
            elif lmList1[4][1] > lmList1[0][1]:  # One finger
                string = ""
                print("Delete")
            # Handle "Thumbs Up" (only trigger result once)
            elif fingers == [1, 0, 0, 0, 1]:  # Thumb higher than usual
                if not thumb_processed or error_detection:  # Process only once
                    try:
